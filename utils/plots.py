...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            #이부분#
            fontt = ImageFont.truetype(font='./font.ttf', size=12)
            
            if label:
                w, h = fontt.getsize(label)  # text width, height
                ...
                self.draw.text((box[0], box[1] - h if outside else box[1]), label, fill=txt_color, font=fontt)
        else:  # cv2
